...
(defn next-seq [conn prefix]
  (let [seq-name (str "_seq." prefix)
...
(defn entity-exists? [conn entity-name]
  (boolean (seq (d/q [:find '?e :where ['?e :name entity-name]] (d/db conn)))))

(defn ensure-entity [conn entity-name]
  (when (not (entity-exists? conn entity-name))
  (d/transact conn [(hash-map :name entity-name)])))

(defn find-descriptor [conn entity-name concept-name]
  (let [r (d/q [:find '?desc-name
...
(defn query-descriptors [conn entity-name]
  (d/q [:find '?concept-name '?val
                 :where ['?e :name entity-name]
                        ['?c :subject '?e]
                        ['?c :predicate :is]
                        ['?c :object '?desc]
...
(defn query-classifications [conn entity-name]
  (let [all-is (d/q [:find '?obj-name
                              :where ['?e :name entity-name]
                                     ['?c :subject '?e]
                                     ['?c :predicate :is]
                                     ['?c :object '?obj]
...
(defn query-relations [conn entity-name]
  (let [all (d/q [:find '?pred '?obj-name
                           :where ['?e :name entity-name]
                                  ['?c :subject '?e]
                                  ['?c :predicate '?pred]
                                  ['?c :object '?obj]
...
(defn query-all-entities [conn]
  (remove (fn [row] (clojure.string/starts-with? (first row) "_seq."))
                   (d/q '[:find ?name :where [?e :name ?name]] (d/db conn))))

(defn query-sources [conn]
  (d/q '[:find ?name ?created-at
                  :where [?e :text _]
                         [?e :name ?name]
                         [?e :created-at ?created-at]]
                (d/db conn)))

(defn query-source-text [conn source-name]
  (let [r (d/q [:find '?t :where ['?e :name source-name] ['?e :text '?t]] (d/db conn))]
             (if (seq r) (ffirst r) nil)))

(defn create-claim [conn subj-name pred obj-name layer]
  (ensure-entity conn subj-name)
  (ensure-entity conn obj-name)
  (d/transact conn [{:subject [:name subj-name]
                              :predicate pred
                              :object [:name obj-name]
                              :layer layer}]))

(defn attach-attr [conn entity-name kind-name val]
  (ensure-entity conn entity-name)
  (ensure-entity conn kind-name)
  (let [existing (find-descriptor conn entity-name kind-name)]
  (if (nil? existing) (let [desc-name (next-seq conn "d")]
  (d/transact conn [(hash-map :name desc-name :value val)])
  (d/transact conn [{:subject [:name entity-name]
                                    :predicate :is
...
(defn query-tasks [conn]
  (d/q '[:find ?task-name ?title-val ?status-val
                  :where
                  [?kc :subject ?task]
                  [?kc :predicate :is]
...
(defn cmd-list []
  ...
  (mapv (fn [row] (let [task-name (first row)
   title (second row)
   stat (nth row 2)
   num (subs task-name 1)]
  (println (str (if (= stat "done") "  [x] #" "  [ ] #") num " " title)))) sorted)))))

(defn cmd-done [id]
  (let [conn (get-conn)
   task-name (str "t" id)
   desc-name (find-descriptor conn task-name "status")]
  (if (nil? desc-name) (println (str "no task #" id)) (do
...
(defn cmd-rm [id]
  (let [conn (get-conn)
   task-name (str "t" id)]
  (if (not (entity-exists? conn task-name)) (println (str "no task #" id)) (do
  (let [task-eid (ffirst (d/q [:find '?e :where ['?e :name task-name]] (d/db conn)))
...
(defn cmd-mint [entity-name kind-arg]
  ...
  (if (nil? kind-arg) (println (str "minted: " n)) (println (str "minted: " n " (" kind-arg ")")))))
...
(defn cmd-about [entity-name]
  (let [conn (get-conn)
   n (str/lower-case entity-name)]
  (if (not (entity-exists? conn n)) (println (str "unknown: " n)) (do
  (println n)
  (let [kinds (query-classifications conn n)]
  (mapv (fn [row] (println (str "  is: " (first row)))) (sort-by first (mapv vec kinds))))
  (let [descs (query-descriptors conn n)]
  (mapv (fn [row] (let [k (first row)
   v (second row)]
  (println (str "  " k ": \"" v "\"")))) (sort-by first (mapv vec descs))))
  (let [rels (query-relations conn n)]
  (mapv (fn [row] (let [p (first row)
   o (second row)]
  (println (str "  " (name p) ": " o)))) (sort-by (fn [r] (name (first r))) (mapv vec rels))))))))

(defn cmd-entities []
  (let [conn (get-conn)
   results (query-all-entities conn)]
  (if (empty? results) (println "no entities.") (let [sorted (sort (mapv first results))]
...
(defn cmd-ingest []
  (let [conn (get-conn)
   ...
   ts (now)]
  (if (empty? raw) (println "nothing to ingest (empty input).") (do
  (d/transact conn [(hash-map :name source-name :text raw :created-at ts)])
  (println (str "ingested " source-name " (" (count raw) " chars)"))))))

(defn cmd-sources []
  (let [conn (get-conn)
   results (query-sources conn)]
  (if (empty? results) (println "no sources.") (let [sorted (sort-by first (mapv vec results))]
  (mapv (fn [row] (let [n (first row)
   ts (second row)]
  (println (str "  " n " " ts)))) sorted)))))

(defn cmd-source [source-name]
  (let [conn (get-conn)
   raw (query-source-text conn source-name)]
  (if (nil? raw) (println (str "unknown: " source-name)) (println raw))))
